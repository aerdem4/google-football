...
class Agent:
    def __init__(self, gc, macro_list, action_counter, dir_cache):
...
class Agent54(Agent):
    def __init__(self, gc, macro_list, action_counter, dir_cache):
        super(Agent54, self).__init__(gc, macro_list, action_counter, dir_cache)
    ...
    def _tactic_foul(self):
        ...
        ball_dist_future = utils.distance(self.gc.controlled_player.pos, self.gc.ball[-1] + ball_speed)

        dist_condition = closest_opp_dist + 0.002 < ball_dist_now + 0.001 < ball_dist_future
        dist_condition = dist_condition and closest_opp_dist < 0.012

        future_opp_pos = closest_opp.get_future_pos(1)
        facing_opp = utils.cosine_sim(future_opp_pos - self.gc.controlled_player.pos,
                                      self.gc.controlled_player.direction) > 0.8
        safe = (not self.gc.controlled_player.yellow) or self.gc.time > 2800

        if safe and dist_condition and facing_opp and (0.0 > closest_opp.pos[0] > -0.7) and (ball_speed[0] < -0.001):
            return True
...
class Agent62(Agent):
    def __init__(self, gc, macro_list, action_counter, dir_cache):
        super(Agent62, self).__init__(gc, macro_list, action_counter, dir_cache)
    ...
    def _last_man_standing(self, safety=0):
        dist = utils.distance(self.gc.controlled_player.pos, self.own_goal)
        distances = [utils.distance(player.pos, self.own_goal) for player in self.gc.players[OWN_TEAM]
                     if player.role != PlayerRole.GoalKeeper]
        if sum([d < dist for d in distances]) <= safety:
            return True
        return False
    ...
    def _decide_clear_ball(self):
        ...
        if closest_opp_dist < 0.1 and self.action_counter[Action.HighPass] > 9:
            return Action.HighPass
        return None
    ...
    def attack(self):
        ...
        if clear_action is not None:
            if self.gc.controlled_player.direction[1] > 0:
                self.dir_cache.register(Action.BottomRight)
            else:
                self.dir_cache.register(Action.TopRight)
            return clear_action
        ...
        if dist_to_goal > 1.5:
            if self.gc.controlled_player.pos[1] > 0:
                return self._run_towards(np.array([-0.2, 0.3]), c=dist_to_goal)
            else:
                return self._run_towards(np.array([-0.2, -0.3]), c=dist_to_goal)
        elif dist_to_goal > 0.2:
