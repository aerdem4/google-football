...
class Agent:
    ...
    def _get_opponent_by_role(self, role):
        return [opp for opp in self.gc.players[OPP_TEAM] if opp.role == role][0]
    ...
    def attack(self):
        ...
        dist_to_gk = utils.distance(self.gc.controlled_player.pos, opp_gk.pos)
        ...
        if self.gc.controlled_player.pos[0] < 0.7:
            return self._run_towards(self.opponent_penalty, c=1.0)
        return self._run_towards(self.opp_goal, c=1.0)
